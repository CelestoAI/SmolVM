...
export function createApp(manager: ConversationManager, staticRoot = fileURLToPath(new URL("../client", import.meta.url))) {
  const server = createServer(async (request, response) => {
    ...
    try { await route(manager, staticRoot, request, response); }
      ...
      const message = status < 500 && error instanceof Error ? error.message : "Smol Agent hit an unexpected error. Check the server log and try again.";
...
function assertLoopbackRequest(request: IncomingMessage): void {
  ...
  if (!new Set(["127.0.0.1", "localhost", "::1"]).has(host)) throw Object.assign(new Error("Smol Agent only accepts requests from this computer."), { status: 403 });
  const site = request.headers["sec-fetch-site"];
  if (site && site !== "same-origin" && site !== "none") throw Object.assign(new Error("Open Smol Agent directly on this computer."), { status: 403 });
}

function cookieValue(request: IncomingMessage): string | undefined {
  return request.headers.cookie?.split(";").map((part) => part.trim()).find((part) => part.startsWith("smol_agent_session="))?.slice("smol_agent_session=".length);
}
...
function assertMutation(request: IncomingMessage): void {
  ...
  if (!origin || !["127.0.0.1", "localhost", "::1"].includes(new URL(origin).hostname)) throw Object.assign(new Error("Reload Smol Agent and try again."), { status: 403 });
  const capability = cookieValue(request)!;
  if (request.headers["x-smol-csrf"] !== sessions.get(capability)) throw Object.assign(new Error("Reload Smol Agent and try again."), { status: 403 });
...
async function main(): Promise<void> {
  ...
  server.listen(port, host, () => console.log(`Smol Agent is ready at http://${host}:${port}`));
}
