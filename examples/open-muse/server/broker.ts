    ...
    try {
      ...
      if (!command.ok) {
        ...
        const snapshotProgram = [
          ...
          "const textTarget = await primary.count().catch(() => 0) ? primary.first() : page.locator('body');",
          "const rawText = sensitivePath ? '' : await textTarget.innerText({ timeout: 5_000 }).catch(() => '');",
