...
export function createWorkflow(apiKey: string, modelId: string): Workflow {
  ...
  return {
    async plan(goal, constraints, signal) {
      if (!apiKey) throw new Error("OPENAI_API_KEY is missing. Add it to .env.local and restart Open Muse.");
      const planner = agentFor([
        "Create exactly three short research steps for the supplied Open Muse goal.",
    ...
    async run(goal, constraints, plan, state) {
      ...
      const buildAgent = agentFor([
        "Build the final Open Muse packet using only the supplied evidence and tools.",
