...
export function toPublicError(error: unknown): PublicError {
  ...
  if (error instanceof Error && error.message.startsWith("OPENAI_API_KEY")) {
    return new PublicError("OpenAI is not configured.", 503, "Add OPENAI_API_KEY to .env.local and restart Open Muse.");
  }
  ...
  if (error instanceof SmolVMError) {
    ...
    return new PublicError(
      "Open Muse could not start a private computer.",
      503,
  ...
  return new PublicError("Open Muse could not finish this research packet.", 500, "Try again.");
}
