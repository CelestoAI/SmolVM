...
function cartPage(cart: CartLine[], review = false): string {
  if (!cart.length) return shell("Cart", `<div class="eyebrow">Your cart</div><h1>Nothing here yet.</h1><div class="empty">Ask OpenMuse to find and add an item.</div>`, 0);
  ...
  const reviewBox = review ? `<div class="review"><div class="eyebrow">Approved preview</div><h2>Order review only</h2><p>No payment or place-order endpoint exists in this demo.</p></div>` : `<button id="checkout">Review checkout</button><p class="muted">OpenMuse must ask before opening review.</p>`;
