...
async function route(manager: RunManager, staticRoot: string, request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (method === "GET" && runMatch) {
    ...
    if (!run) throw Object.assign(new Error("That OpenMuse Research run was not found."), { status: 404 });
  ...
  if (method === "GET" && packetMatch) {
    ...
    response.setHeader("content-disposition", "attachment; filename=OpenMuse-Research-packet.zip");
    response.end(Buffer.from(bytes));
    return;
  }
  if (url.pathname.startsWith("/api/")) throw Object.assign(new Error("That OpenMuse Research route does not exist."), { status: 404 });
...
function assertMutationRequest(request: Pick<IncomingMessage, "headers">, expectsJson: boolean): void {
  ...
  if (!loopbackHosts.has(hostname)) {
    throw Object.assign(new Error("OpenMuse Research only accepts requests from this computer."), { status: 403 });
  }
...
function streamEvents(manager: RunManager, id: string, request: IncomingMessage, response: ServerResponse): void {
  ...
  if (!unsubscribe) {
    response.end(`event: error\ndata: ${JSON.stringify({ error: "That OpenMuse Research run was not found." })}\n\n`);
    return;
  }
...
async function main(): Promise<void> {
  ...
  server.listen(config.port, config.host, () => console.log(`OpenMuse Research is ready at http://${config.host}:${config.port}`));
}
