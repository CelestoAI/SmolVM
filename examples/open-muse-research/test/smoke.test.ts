...
test("packed SDK creates a real VM, exports a deterministic packet, and cleans up", {
  skip: process.env.OPEN_MUSE_RESEARCH_REAL_VM !== "1" ? "set OPEN_MUSE_RESEARCH_REAL_VM=1 on a supported host" : false,
  timeout: 10 * 60_000,
}, async () => {
  const workflow: Workflow = {
    ...
    run: async (_goal, _constraints, _plan, state) => {
      ...
      await state.sandbox.files.write("/workspace/open-muse-research/output/sources.json", JSON.stringify(source));
      ...
      await state.sandbox.files.write("/workspace/open-muse-research/output/itinerary.md", "# Day 1\nFood [source:S01]\n# Day 2\nArchitecture [source:S01]\n# Day 3\nWalk [source:S01]");
      await state.sandbox.files.write("/workspace/open-muse-research/research/budget-input.json", JSON.stringify([{ category: "food", item: "Meals", quantity: 2, unitCostInr: 1000, sourceId: "S01" }]));
      const result = await state.sandbox.exec(["python3", "/workspace/open-muse-research/scripts/calculate_budget.py", "/workspace/open-muse-research/research/budget-input.json", "/workspace/open-muse-research/output/budget.csv"]);
      assert.equal(result.ok, true, result.stderr);
    },
  };
  const manager = new RunManager(workflow);
  const plan = await manager.createPlan("Build the deterministic OpenMuse Research smoke packet.", []);
