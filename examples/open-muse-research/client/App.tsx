...
export function App() {
  ...
  return <main className="shell">
    <header className="app-header"><div className="brand"><span className="brand-mark">M</span><strong>OpenMuse Research</strong></div><div className="local-status"><span />{running ? `Running locally · ${run?.phase.replaceAll("_", " ")}` : "Local demo · ready"}</div></header>
    ...
    <section className="workspace">
      <header className="workspace-header"><div><p className="eyebrow">Isolated workspace</p><h2>Research computer</h2></div><div className={`phase-pill ${running ? "active" : ""}`}><span />{run?.phase.replaceAll("_", " ") ?? "ready"}</div></header>
      {error && <div className="error-banner" role="alert"><strong>OpenMuse Research needs attention</strong><span>{error}</span><button onClick={() => setError(undefined)}>Dismiss</button></div>}
      {finalSnapshotError && <div className="error-banner" role="alert"><strong>OpenMuse Research needs attention</strong><span>{finalSnapshotError}</span><button onClick={() => setFinalSnapshotRetry((value) => value + 1)}>Retry</button></div>}
