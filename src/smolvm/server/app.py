    ...
    def browser_response(browser: DisplaySandboxProtocol) -> BrowserSessionResponse:
        if browser.cdp_url is None:
            raise _sdk_error(
                ...
                f"Browser session '{browser.session_id}' has no automation endpoint; delete it and create the session again.",
            )
    ...
    def create_browser_session(body: CreateBrowserSessionRequest) -> BrowserSessionResponse:
        ...
        try:
            ...
            raise _sdk_error(
                400,
                code,
                f"Could not create browser session '{session_id}': {exc}. Fix the options and try again.",
            ) from exc
        publish(
            {
                "type": "browser.ready",
    ...
    def delete_browser_session(session_id: str) -> Response:
        ...
        try:
            ...
            raise _sdk_error(
                409,
                "cleanup_failed",
                f"Browser session '{session_id}' could not be deleted; call browser.delete() again, or close the SmolVM client.",
            ) from exc
        finally:
            with suppress(Exception):
                browser.close()
